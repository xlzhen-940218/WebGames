...
- `games/tanchishe.html`: 贪吃蛇小游戏页面。
- `games/eluosifangkuai.html`: 俄罗斯方块小游戏页面。
- `images/`: 游戏使用的图片资源目录。
...
- 贪吃蛇（`games/tanchishe.html`）：玩家控制一条移动的蛇去吃场景中出现的食物。每吃到一个食物蛇身会变长并增加得分；蛇不能撞到自己的身体或地图边界，否则游戏结束。游戏通常通过方向键控制移动方向，难度可随时间或得分提高（蛇速加快）。主要业务目标是尽可能获取更高分数并生存更久。
- 俄罗斯方块（`games/eluosifangkuai.html`）：由不同形状的方块（方块、直线、L 形等）从顶部连续下落，玩家可以左右移动并旋转方块以合理放置。若某一行被填满则该行被消除并计分，消除多行或连续消除通常得分更高。游戏以堆叠高度达到顶部为结束条件，常通过逐步加快下落速度来提升难度。业务目标是通过合理排列和消除方块获得高分并延长游戏时间。
